...
socket.on('posUpdate', (data) => {
    ...
    if (Object.keys(localPlatforms).length === 0 && data.platforms && platformImageLoaded) { 
        data.platforms.forEach((platform, index) => {

            let platformTopY = platform.y - (platform.h / 2);
                ...
                { 
                    ...
                    render: {
                        sprite: {
                            ...
                            xScale: platform.w / platformImage.width, 
                            yScale: 80 / platformImage.height, 
                        }
                    }
                }
            );
            localPlatforms[index] = body; 
            World.add(world, body); 
        });
    }
    const serverPlayerIds = Object.keys(data.positions);
    ...
    for (const id of serverPlayerIds) {
        ...
        if (localPlayers[id]) 
        {
            Matter.Body.setPosition(localPlayers[id], serverPos);
        } 
        else 
        {

            let newPlayerBody = Bodies.rectangle(
                serverPos.x, 
                serverPos.y, 
                100, 
                100,
            ...
            localPlayers[id] = newPlayerBody;
            World.add(world, newPlayerBody);
        }
    }


    const localPlayerIds = Object.keys(localPlayers);
    for (const id of localPlayerIds) {
        if (!data.positions[id]) {
            World.remove(world, localPlayers[id]);
